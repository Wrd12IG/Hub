...
/* ─────────────────────────────────────────────────────────────────
   MODAL: WINDSOR.AI REPORTING IDS (Instagram / Search Console / LinkedIn organic)
   Plain account ids used only by the "Report Marketing" tab — no OAuth, the
   data itself is pulled through the agency's Windsor.ai account, not a
   per-client token. See lib/reporting.ts / lib/windsor-client.ts.
───────────────────────────────────────────────────────────────── */

function WindsorFieldModal({
  clientId, title, fieldKey, currentValue, placeholder, hint, accentColor, onClose, onSaved,
}: {
  clientId: string;
  title: string;
    ...
    <div className="w-full space-y-6">
        ...
        <div className="rounded-2xl border p-5 space-y-3 transition-all duration-200" style={cardStyle(!!client.windsorAccounts?.searchconsole, "#4285F4")}>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              ...
              <span className="text-sm font-bold text-foreground">Search Console</span>
            </div>
        ...
        <div className="rounded-2xl border p-5 space-y-3 transition-all duration-200" style={cardStyle(!!client.windsorAccounts?.linkedin_organic, "#34A853")}>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              ...
              <span className="text-sm font-bold text-foreground">LinkedIn (Report)</span>
            </div>
      ...
      {activeModal === "windsor-instagram" && (
        <WindsorFieldModal
          ...
          fieldKey="instagram"
          currentValue={client.windsorAccounts?.instagram}
          placeholder="es. 17841407927101615"
          hint="Lo trovi nel pannello Windsor.ai (onboard.windsor.ai) sotto il connector Instagram."
          accentColor="#E1306C"
          onClose={() => setActiveModal(null)}
          onSaved={() => { refreshClient(); setActiveModal(null); }}
        />
      )}
      {activeModal === "windsor-searchconsole" && (
        <WindsorFieldModal
          clientId={clientId}
          title="Search Console — URL sito verificato"
          fieldKey="searchconsole"
          currentValue={client.windsorAccounts?.searchconsole}
      ...
      {activeModal === "windsor-linkedin" && (
        <WindsorFieldModal
          clientId={clientId}
          title="LinkedIn — Organization ID (per Report Marketing)"
          fieldKey="linkedin_organic"
          currentValue={client.windsorAccounts?.linkedin_organic}
          placeholder="es. 53113725"
          hint="Numero dell'Organization ID LinkedIn collegato su Windsor.ai — non è legato alla connessione LinkedIn qui sopra."
          accentColor="#34A853"
          onClose={() => setActiveModal(null)}
          onSaved={() => { refreshClient(); setActiveModal(null); }}
        />
